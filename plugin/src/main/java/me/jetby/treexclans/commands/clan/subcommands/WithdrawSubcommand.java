...
public class WithdrawSubcommand implements Subcommand {
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull String[] args) {
        ...
        if (sender instanceof Player player) {
            if (args.length == 0) {
                plugin.getMessages().sendMessage(player, null, "commands.withdraw");
                return true;
            }
            ...
            if (plugin.getClanManager().economy().getBalance(clanImpl) >= balance) {
                ...
                plugin.getMessages().sendMessage(player, clanImpl, "clan-balance-withdraw", new Messages.ReplaceString("{sum}", String.valueOf(balance)));
            } else {
